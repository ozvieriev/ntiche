...
angular.module('app.controllers').controller('accountSignUpController', ['$scope', '$state', '$form', '$auth', function ($scope, $state, $form, $auth) {
  ...
  $scope.submit = function (form) {
    $form.submit($scope, form, function () {
      return $auth.signUp(angular.copy($scope.model)).then(function () {
        $auth.signIn($scope.model.userName, $scope.model.password).then(function () {
          if ($state.params.returnUrl) $state.go($state.params.returnUrl);else $state.go('account/index');
        });
      }, function (error) {
        $scope.status = error.status;
