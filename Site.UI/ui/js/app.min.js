...
angular.module('app.auth').factory('$auth', ['$q', '$injector', '$authStorage', function ($q, $injector, $authStorage) {
  ...
  service.emailConfirmation = function (token, account) {
    var data = {
      grant_type: 'password',
  ...
  service.resetPassword = function (token, account, password) {
    var $http = $injector.get("$http");
...
angular.module('app.controllers').controller('accountEmailConfirmationController', ['$scope', '$auth', '$utils', function ($scope, $auth, $utils) {
  ...
  if (!query.token || !query.account) return $scope.status = 404;
...
angular.module('app.controllers').controller('accountResetPasswordController', ['$scope', '$state', '$form', '$utils', '$auth', function ($scope, $state, $form, $utils, $auth) {
  $scope.model = {};
  $scope.status = null;
  $scope.isBusy = false;
  var query = $utils.query();
  ...
  $scope.submit = function (form) {
    $scope.status = null;
    $form.submit($scope, form, function () {// return $auth.recoverPassword({ email: $scope.model.email })
