...
angular.module('app').config(['$httpProvider', function ($httpProvider) {
  $httpProvider.interceptors.push(['$q', '$translate', '$state', function ($q, $translate, $state) {
    var interceptor = {};
    ...
    interceptor.responseError = function (response) {
      if (response.config.asJson === true) {
        ...
        if (!response.data.error_description) $state.go('error');
...
angular.module('app.auth').factory('$auth', ['$q', '$injector', '$authStorage', function ($q, $injector, $authStorage) {
  var _apiHeaders = {
  ...
  service.signIn = function (userName, password, emailConfirmationToken, accountId) {
    var data = {
      grant_type: 'password'
    };
...
angular.module('app.services').factory('$api', ['$http', '$state', function factory($http, $state) {
  ...
  service.examPost = function (answers) {
    return $http.post(_uri('api/exam'), answers, {
      asJson: true
    });
  };
...
angular.module('app.controllers').controller('examIndexController', ['$scope', '$sce', '$form', '$api', function ($scope, $sce, $form, $api) {
  $scope.model = {};
  $scope.status = null;
  $scope.description = null;
  $scope.isBusy = false;
  $scope.isAnySelected = false;
  $api.exam().then(function (response) {
  ...
  $scope.submit = function (form) {
    $form.submit($scope, form, function () {
      ...
      return $api.examPost(answers).then(function (response) {
        $scope.status = 200;
        $scope.description = response.description;
      }, function (error) {
        $scope.status = error.status;
...
angular.module('app.controllers').controller('partialHeaderController', ['$scope', '$state', '$auth', function ($scope, $state, $auth) {
  $scope.$auth = $auth;
