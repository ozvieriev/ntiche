...
angular.module('app', ['app.components', 'app.auth', 'app.services', 'app.controllers', 'app.directives', 'app.filters']).run(function ($transitions, $translate, $templateCache, $auth) {
...
angular.module('app').config(['$httpProvider', function ($httpProvider) {
  $httpProvider.interceptors.push(['$q', '$state', function ($q, $state) {
    ...
    interceptor.request = function (config) {
      if ($state.params && $state.params.locale) config.headers['Accept-Language'] = $state.params.locale;
      return config;
    };
...
angular.module('app.auth').factory('$auth', ['$q', '$injector', '$authStorage', '$state', function ($q, $injector, $authStorage, $state) {
  ...
  service.isInRoles = function (roles) {
    var identity = service.identity();
    if (roles && !identity) return false;
...
angular.module('app.directives').directive('ngLoadingForm', ['$window', function ($window) {
  return {
    ...
    scope: {},
    link: function link(scope, element, attrs) {
      var $submit = element.find('[type="submit"]:first');
      $submit.addClass('btn btn-primary').prepend('<span class="spinner-border spinner-border-sm mr-2 ng-hide"></span>');
      var $span = $submit.find('span:first');
      var watcher = scope.$watch('$parent.isBusy', function (isBusy) {
        isBusy ? element.attr('disabled', 'disabled') : element.removeAttr('disabled');
        isBusy ? $submit.addClass('btn-light') : $submit.removeClass('btn-light');
        isBusy ? $span.removeClass('ng-hide') : $span.addClass('ng-hide');
      });
...
angular.module('app.services').factory('$api', ['$http', function factory($http) {
  ...
  service.accoutnActivity = function () {
...
angular.module('app.services').factory('$dict', ['$sce', '$filter', function ($sce, $filter) {
  var _questions = function _questions() {
    return [{
...
angular.module('app.controllers').controller('accountEmailConfirmationController', ['$scope', '$auth', '$state', '$timeout', function ($scope, $auth, $state, $timeout) {
  ...
  var timer = $timeout(function () {
    if ($scope.isBusy) $scope.view = 'loading';
  }, 1000);
  $scope.$on('$destroy', function () {
    $timeout.cancel(timer);
  });
}]);
angular.module('app.controllers').controller('accountController', ['$scope', function ($scope) {
  $scope.model = {};
  $scope.status = null;
  $scope.description = null;
  $scope.isBusy = true; //$api.examResults().then(
  //    (response) => {
  //        $scope.status = 200;
...
angular.module('app.controllers').controller('adminReportController', ['$scope', '$auth', '$state', function ($scope, $auth, $state) {
  ...
  var accountTo = ('0' + (tomorrow.getMonth() + 1)).slice(-2) + '/' + ('0' + tomorrow.getDate()).slice(-2) + '/' + tomorrow.getFullYear();
  $scope.model = {
  ...
  var _getModel = function _getModel() {
    ...
    var from = Date.parse($scope.model.accountFrom);
    var to = Date.parse($scope.model.accountTo);
  ...
  $scope.downloadExamReport = function () {
    window.open("/api/exam/report/?".concat($.param(_getModel())), '_blank');
  };

  $scope.downloadEvaluationReport = function () {
    window.open("/api/feedback/report/?".concat($.param(_getModel())), '_blank');
  };
}]);
...
angular.module('app.controllers').controller('examController', ['$scope', '$state', '$api', '$timeout', function ($scope, $state, $api, $timeout) {
  ...
  $scope.isBusy = true;
  $scope.view = null;
  $api.accoutnActivity().then(function (response) {
    $scope.isBusy = false;
...
angular.module('app.controllers').controller('examPostTestController', ['$scope', '$form', '$api', '$dict', '$state', function ($scope, $form, $api, $dict, $state) {
  var _exam = 'post-test';
  $scope.model = {
  ...
  $scope.onSelect = function () {
    if ($scope.isAllSelected) return;
    var questions = $scope.model.questions;

    for (var index = 0; index < questions.length; index++) {
      if (!questions[index].selectedAnswer) return;
    }
  ...
  $scope.submit = function (form) {
    $form.submit($scope, form, function () {
      ...
      for (var index = 0; index < questions.length; index++) {
        ...
        json["answer".concat(index)] = question.answers.indexOf(selectedAnswer);
      }

      json.percentCorrect = Math.round(correctAnswers / questions.length * 100);
      return $api.examPost(_exam, json).then(function (response) {
        $scope.status = 200;
        $scope.description = response.description;

        if (response.isSuccess) // && !response.TotalFeedbacks
          {
            $scope.view = response.TotalFeedbacks ? 'done-success' : 'done-feedback'; //return $state.go(`exam/feedback`, { examResultId: response.examResultId })
          } else $scope.view = 'done-fail';
      }, function (error) {
        $scope.status = error.status;
        $scope.description = error.data.error_description;
      }).finally(function () {
...
angular.module('app.controllers').controller('examPreTestController', ['$scope', '$form', '$api', '$dict', function ($scope, $form, $api, $dict) {
  var _exam = 'pre-test';
  $scope.model = {
    questions: $dict.questions()
  };
